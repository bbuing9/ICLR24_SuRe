...
from functions.gen_candidates import post_process_candidate, separation, use_api_candidate
...
def sure_infer(model, dataset, iters=1, n_articles=10, output_path='./'):
    ...
    if not os.path.exists(path_to_candidate):
        sure_candidate = use_api_candidate(model, dataset, iters=iters, n_articles=n_articles)
    ...
    if not os.path.exists(path_to_summary1):
        summary_candidate1 = use_api_summary(model, dataset, sure_candidate_post, pred=0, n_articles=n_articles)
    ...
    if not os.path.exists(path_to_summary2):
        summary_candidate2 = use_api_summary(model, dataset, sure_candidate_post, pred=1, n_articles=n_articles)
...
def error_check_fin(res):
    corner_cases = ['Cannot', 'False', 'Unknown']
